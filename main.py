...
def monthly_csv(workbook: str):
    ...
    for month, sheet_name in enumerate(sheets, start=1):
        ...
        monthly = monthly.loc[: (days - 1), :"Развлечения"]
        # ПОЯСНЕННЯ: нижня межа табличних даних завжди буде визначатися як: ("кількість днів у поточному місяці" - 1),
        # тому що мітки рядків починаються з "0". Ліва межа табличних даних завжди буде обмежуватися стовбчиком: "Развлечения".
        ...
        names_mapping = {
            ...
            "Зарплата": "salary",
            "Другие доходы": "other_income",
            "Питание": "food",
            "Проезд": "travel",
            "Здоровье": "health",
            "Хоз.Товары": "household_goods",
            "Квартира": "apartment",
            "Долги": "debts",
            "Вещи": "clothes_shoes",
            "Другое": "other_expenses",
            "Алкоголь": "alcohol",
            "Закуски": "tasties",
            "Мероприятия": "events",
        ...
        # Зберігаємо оброблені дані за місяць року у форматі '.csv'
        monthly.to_csv(
...
def prepare_sql_tables(engine):
    print("Запускаю SQL-скрипт з початковими аналітичними розрахунками...")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Шлях до директорії з робочими книгами Excel",
    )
    parser.add_argument(
        "-y",
        "--year",
    ...
    # Створення логіки обробки даних, введених користувачем
    ...
    print("*********************************************")
    print("* II. ОПИСОВА СТАТИСТИКА ТА РОЗПОДІЛИ ДАНИХ *")
    print("*********************************************")
